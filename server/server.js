...
function nextState(state, x, y) {
    ...
    let res;
    if (state.stage === 'go') {
        if (isCorner(x, y)) return null;          // 四角禁落
        res = goMove(state, x, y);
        if (!res) return null;                    // 非法落子
        newState.board = res.board;
        newState.capturedScore = res.capturedScore;
...
function broadcastOnlineCount(roomId) {
    ...
    if (!room) return;
    const count = room.players.filter(p => p).length + room.spectators.size;
...
io.on('connection', socket => {
    socket.on('joinRoom', ({ roomId, side }) => {
        if (!rooms.has(roomId)) {
            ...
            socket.join(roomId);
            socket.emit('joined', { roomId, side: 'black', role: 'player' });
            broadcastOnlineCount(roomId); // ✅新增
            return;
        }
        const room = rooms.get(roomId);
        if (room.players[0] && room.players[1]) {
            ...
            socket.join(roomId);
            socket.emit('joined', { roomId, side: null, role: 'spectator' });
        ...
        socket.join(roomId);
        socket.emit('joined', { roomId, side: 'white', role: 'player' });
    ...
    socket.on('move', ({ roomId, x, y }) => {
        ...
        if (newState.over) {
            finalize(newState);                       // ①结算
            io.to(roomId).emit('gameOverToast', newState); // ②广播
        }
    ...
    socket.on('pass', roomId => {
        ...
        const whoPassed = state.turn === 'black' ? '黑方' : '白方';
        state.passCount++;
        if (state.passCount === 2) {
            state.stage = 'othello';
            state.turn = 'black';
            state.passCount = 0;
            io.to(roomId).emit('passToast', whoPassed); // ✅提前
            io.to(roomId).emit('update', state);
            return;
        }

        io.to(roomId).emit('passToast', whoPassed); // ✅提前
        state.turn = state.turn === 'black' ? 'white' : 'black'; // 再切换
        state.move++;
        if (state.move >= 60 && state.stage === 'go') {
            state.stage = 'othello'; state.turn = 'black';
        }
        if (state.over) finalize(state);
        io.to(roomId).emit('update', state);
    });
